...
Wx  = theano.shared(0.2 * numpy.random.uniform(-1.0, 1.0,\
		   (de * cs, nh)).astype(theano.config.floatX), name='Wx')
Wh  = theano.shared(0.2 * numpy.random.uniform(-1.0, 1.0,\
		   (nh, nh)).astype(theano.config.floatX), name='Wh')
W   = theano.shared(0.2 * numpy.random.uniform(-1.0, 1.0,\
		   (nh, nc)).astype(theano.config.floatX), name='W')
...
def recurrence(x_t, h_tm1):
	h_t = T.dot(x_t, Wx) + T.dot(h_tm1, Wh)
