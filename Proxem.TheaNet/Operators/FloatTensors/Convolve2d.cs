...
{
    public class Convolve2d : Tensor<float>.Binary<Tensor<float>, Array<float>, Tensor<float>, Array<float>>
    {
        public readonly ConvMode mode;
        private Dim[] _shape;

        public Convolve2d(Tensor<float> x, Tensor<float> y, ConvMode mode = ConvMode.Full) :
            base("Convolve", x, y, mode.Named("mode"))
        {
            if (x.NDim != 2 && y.NDim != 2) throw new RankException("Expect inputs of dim 2");
            this.mode = mode;
            _shape = new Dim[] { GetConvolveDim(x.Shape[0], y.Shape[0], mode)  };
        }
        ...
        {
            bp.PushGradientTo(x, new Correlate2d(delta, y, Reverse(mode)));
            bp.PushGradientTo(y, new Correlate2d(delta, x, Reverse(mode)));
        }

        public override Binary<Tensor<float>, Array<float>, Tensor<float>, Array<float>> Clone(Tensor<float> x, Tensor<float> y) =>
            new Convolve2d(x, y, mode);
    }
}
